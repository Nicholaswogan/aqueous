...
cdef load_spronsbl():
  ...
  path_c = path
  path_len = len(path)
  pxd.aqueous_load_spronsbl(&path_len, path_c)
...
cpdef api double gibbs_energy(str species, double T, double P) except? -1.0:
  ...
  pxd.aqueous_gibbs_energy(&species_len, species_c, &T, &P, &err_len, &err, &G)
  if err:
...
cdef class AqueousSolution:
  ...
  def __dealloc__(self):
    pxd.aqueous_dealloc_aqueoussolution(&self._ptr)
    
  property xtol:
    def __get__(self):
      cdef double val;
      pxd.aqueous_aqueoussolution_xtol_get(&self._ptr, &val)
      return val
  ...
  property conserv_tol:
    def __get__(self):
      cdef double val;
      pxd.aqueous_aqueoussolution_conserv_tol_get(&self._ptr, &val)
      return val
    def __set__(self, double val):
      pxd.aqueous_aqueoussolution_conserv_tol_set(&self._ptr, &val)
      
  property G_init:
    def __get__(self):
      cdef double val;
      pxd.aqueous_aqueoussolution_g_init_get(&self._ptr, &val)
      return val
      
  property G_opt:
    def __get__(self):
      cdef double val;
      pxd.aqueous_aqueoussolution_g_opt_get(&self._ptr, &val)
      return val
      
  property algorithm:
    def __get__(self):
      cdef char val[STR_LEN];
      pxd.aqueous_aqueoussolution_algorithm_get(&self._ptr, val)
      return val.decode()
    def __set__(self, str val):
      ...
      val_c = val_b
      val_len = len(val)
      pxd.aqueous_aqueoussolution_algorithm_set(&self._ptr, &val_len, val_c)  
