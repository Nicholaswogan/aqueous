...
contains
  ...
  subroutine process_datafile(root, dat, x, err)
    ...
    do j = 1,dat%nsp
      ind = findloc(as%species_names,dat%species_names(j),1)
      if (ind == 0) then
        ind = findloc(as%alt_names,dat%species_names(j),1)
        if (ind == 0) then
          err = '"'//trim(dat%species_names(j))//'" is not in the data base.'
          return 
        else
          ! check that entry is unique
          do i = 1,as%nsp
            if (i == ind) cycle
            if (as%alt_names(ind) == as%alt_names(i)) then
              if (.not. allocated(dups)) then
                dups = '{unique-name: "'//trim(as%species_names(ind))//'", alt-name: "'//trim(as%alt_names(ind))//'"}'
              endif
              dups = dups//', {unique-name: "'//trim(as%species_names(i))//'", alt-name: "'//trim(as%alt_names(i))//'"}'
            endif
          enddo
          if (allocated(dups)) then
            err = '"'//trim(dat%species_names(j))//'" is not a unique entry in the database. '//dups
            return
          endif
        endif
      endif
  ...
  subroutine load_spronsbl()
    ...
    root => parse("../spronsbl.yaml", error = error)
    if (error/='') then
      print*,trim(error)
      stop 1
    endif
